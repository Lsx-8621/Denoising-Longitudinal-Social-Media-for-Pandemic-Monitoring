...
class DualGraphConvolution(nn.Module):
    ...
    def forward(self, text, dep_adj, latent_adj=None, use_activation=True):
        ...
        # text: [batch_size, sequence_len, in_features]
        # weight: [in_features, out_features]
        # hidden: [batch_size, sequence_len, out_features]
        hidden = torch.matmul(text, self.weight)  
        # 对依存关系矩阵进行归一化
        # dep_adj： [batch_size, sequence_len, sequence_len]
        # denom： [batch_size, sequence_len, 1]
        # output: [batch_size, sequence_len, out_features]
...
class GAT(nn.Module):
    ...
    def __init__(self, opt, in_dim, hidden_size=256, mem_dim=600, num_layers=2):
        ...
        # Leaky ReLU在输入为负数时输出一个小的非零值而不是0
...
class Classifier(nn.Module):
    ...
    def forward(self, sentence, bert_out, adj, aspect_double_idx, text_len, aspect_len, syntax_distance=None,):
        ...
        #old_weighted_x = weighted_x
        
        #weighted_x = self.position_weight(x, aspect_double_idx, text_len, aspect_len)
        
        #gate_x = old_weighted_x.sigmoid()
        
        weighted_x = x #gate_x * weighted_x  + (1.0 - gate_x) * old_weighted_x
        
        x = self.gc2(weighted_x, adj) #gc2(x, rl_adj)
        #'''
        #x = self.gat(weighted_x, rl_adj)
    
        
        gcn_x = x 
        '''
        aspect_x = self.mask_nonaspect(sentence, aspect_double_idx)
        alpha_mat = torch.matmul(aspect_x, x.transpose(1, 2))
        alpha = F.softmax(alpha_mat.sum(1, keepdim=True), dim=2)
        x = torch.matmul(alpha, x).squeeze(dim=1) # batch_size x 2*hidden_dim
        '''
        aspect_x = self.mask_nonaspect(x, aspect_double_idx)
        
        alpha_mat = torch.matmul(aspect_x, sentence.transpose(1, 2))

        #print(syntax_distance) 
        #sys.exit(0)
        ...
        if bert_out is not None:
            ...
            #    alpha_mat_mixed = alpha_mat_mixed + rank_logits.unsqueeze(dim=1)
            
            #alpha_mat_mixed = alpha_mat_mixed + 0.1 * syntax_distance.unsqueeze(dim=1)
        ...
        #print(f"[tlog] alpha: {alpha.size()}")
        #print(f"[tlog] x: {x.size()}")
        #sys.exit(0)
        #x = torch.matmul(alpha, x).squeeze(dim=1) # batch_size x 2*hidden_dim
        #masked_sentence = syn_dist_mask.unsqueeze(dim=-1) * sentence
        
        #x = torch.matmul(alpha, masked_sentence).squeeze(dim=1) 
        x = torch.matmul(alpha, sentence).squeeze(dim=1) 
        
        #mlp_output = aspect_x.mean(dim=-1)
        mlp_output = x 
...
class DepGCNv2(nn.Module):
    ...
    def forward(self, inputs, labels = None,  debugger=None, temperature=None):
        ...
        #print(str(merged_layer.size()))
        #sys.exit(0)
        #merged_layer = merged_layer + pooled_output[:,None,:]
